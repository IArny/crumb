...
internals.schema = Joi.object().keys({
    ...
    generateCrumb: Joi.boolean().optional(),
    checkCrumbValue: Joi.boolean().optional()
});
...
internals.defaults = {
    ...
    generateCrumb: false,           // Specify function to replace csrf cookie generation method
    checkCrumbValue: false          // Specify function to replace csrf token check method
};
...
exports.plugin = {
    ...
    register: function (server, options) {
        ...
        server.ext('onPostAuth', (request, h) => {
            ...
            const checkCrumbValue = (crumbValue) => {
                if (typeof settings.checkCrumbValue === 'function') {
                    return settings.checkCrumbValue(crumbValue)
                } else {
                    return crumbValue === getCrumbValue();
                }
            };
        ...
        const generate = settings.generateCrumb === 'function' ? settings.generateCrumb : (request, h) => {
